  ...
  describe("Pool that borrows ERC20 with ERC20 as collateral", async () => {
    ...
    describe("Amount lent > minBorrowAmount at the end of collection period", async () => {
      ...
      beforeEach(async () => {
        ...
        await borrowToken
          .connect(admin)
          .transfer(random.address, BigNumber.from(10).pow(22));
      });
      ...
      it("Lender tokens should be transferable", async () => {
        ...
        await poolToken
          .connect(lender)
          .transfer(random.address, transferAmount);
      ...
      it("Pool cannot be cancelled by anyone", async () => {
        await expect(pool.connect(borrower).cancelPool()).to.be.revertedWith(
          "CP1"
        );

        await expect(pool.connect(lender).cancelPool()).to.be.revertedWith(
          "CP1"
        );

        await expect(pool.connect(random).cancelPool()).to.be.revertedWith(
          "CP1"
        );
      });
      ...
      context("Borrower should repay interest", async () => {
        it("Repay interest for first  repay period", async () => {
          ...
          const interestForCurrentPeriod =
            await repaymentImpl.getInterestDueTillInstalmentDeadline(
              pool.address
            );
          assert(
            interestForCurrentPeriod
              .div(BigNumber.from(10).pow(30))
              .toString() == repayAmount.toString(),
            `Incorrect interest for period 1. Actual: ${interestForCurrentPeriod
              .div(BigNumber.from(10).pow(30))
              .toString()} Expected: ${repayAmount.toString()}`
          );
          await borrowToken
            .connect(random)
            .approve(repaymentImpl.address, repayAmount);
          await repaymentImpl
            .connect(random)
            .repayAmount(pool.address, repayAmount);
        });
        return;

        it("Can repay for second repayment period in first repay period", async () => {
          const repayAmount = createPoolParams._borrowRate
            .mul(createPoolParams._borrowAmountRequested)
            .mul(createPoolParams._repaymentInterval)
            .div(60 * 60 * 24 * 356)
            .div(BigNumber.from(10).pow(30));
          ...
          await borrowToken
            .connect(random)
            .approve(repaymentImpl.address, repayAmount.add(10));
          await repaymentImpl
            .connect(random)
            .repayAmount(pool.address, repayAmount.add(10));
        });

        it("Repay in grace period, with penality", async () => {
          const endOfPeriod: BigNumber =
            await repaymentImpl.getNextInstalmentDeadline(pool.address);
          ...
          await timeTravel(
            network,
            parseInt(endOfPeriod.add(gracePeriod).sub(10).toString())
          );
          ...
          const repayAmount = createPoolParams._borrowRate
            ...
            .div(60 * 60 * 24 * 356)
            .div(BigNumber.from(10).pow(30));
          const repayAmountWithPenality = repayAmount.add(
            repaymentParams.gracePenalityRate.mul(
              await repaymentImpl.getInterestLeft(pool.address)
            )
          );
          await borrowToken
            .connect(random)
            .approve(repaymentImpl.address, repayAmountWithPenality);
          await expect(
            repaymentImpl.connect(random).repayAmount(pool.address, repayAmount)
          ).to.be.revertedWith("");
          await repaymentImpl
            .connect(random)
            .repayAmount(pool.address, repayAmountWithPenality);
        });
        ...
        it("Repay for next period after repayment in grace period", async () => {
          ...
          await repaymentImpl
            .connect(random)
            .repayAmount(pool.address, repayAmountWithPenality.add(20));
          const interestForCurrentPeriod =
            await repaymentImpl.getInterestDueTillInstalmentDeadline(
              pool.address
            );
          assert(
            interestForCurrentPeriod.toString() ==
              repayAmount.sub(20).toString(),
      ...
      context("Borrower requests extension", async () => {
        ...
        it("Extension passed", async () => {
          ...
          assert(
            await repaymentImpl.isLoanExtensionActive(),
        ...
        context("Extension passed", async () => {
          ...
          it("liquidate if repay less than interest for extended period", async () => {
            ...
            await repaymentImpl.repayAmount(
              pool.address,
              interestForCurrentPeriod.sub(1)
            );

            const endOfExtension: BigNumber =
              await repaymentImpl.getNextInstalmentDeadline(pool.address);
            await timeTravel(
              network,
              parseInt(endOfExtension.add(1).toString())
            );
          ...
          it("Can't liquidate if repay is more than interest for extended period", async () => {
            ...
            await borrowToken
              .connect(random)
              .approve(repaymentImpl.address, interestForCurrentPeriod);
            await repaymentImpl
              .connect(random)
              .repayAmount(pool.address, interestForCurrentPeriod);
        ...
        context("Extension failed", async () => {
          it("Shouldn't be liquidated for current period if interest is repaid", async () => {
            ...
            await timeTravel(
              network,
              parseInt(extensionVoteEndTime.add(1).toString())
            );
      ...
      context(
        "Margin call when collateral ratio falls below ideal ratio",
        async () => {
          beforeEach(async () => {
            await priceOracle
              .connect(admin)
              .setfeedAddress(Contracts.LINK, ChainLinkAggregators["DAI/USD"]);
          });
          ...
          it("Margin call can't be liquidated, if borrower adds collateral for margin call", async () => {
            ...
            const amount: BigNumber = createPoolParams._poolSize.sub(
            ...
            await pool
              .connect(borrower)
              .addCollateralInMarginCall(lender.address, amount, false);
          ...
          context(
            "Collateral added in margin call is specific to lender",
            async () => {
