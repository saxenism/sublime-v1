...
contract Pool is Initializable, IPool, ReentrancyGuard {
    ...
    function cancelIncompletePool() external {
        require (
    ...
    function cancelPool() external {
        ...
        penality = IPoolFactory(PoolFactory).poolCancelPenalityFraction().mul(_collateralLiquidityShare).div(10**8);
    }
    ...
    function _cancelPool(uint256 _penality) internal {
        ...
        withdrawAllCollateral(_penality);
        poolToken.pause();
    ...
    function liquidateCancelPenality(
        ...
        IERC20(poolConstants.borrowAsset).trasferFrom(msg.sender, _liquidationTokens);
    ...
    function liquidateLender(
                ...
                );
        {
            uint256 _lenderLiquidationTokens =
                correspondingBorrowTokens(_lenderCollateralShare, _poolFactory, IPoolFactory(_poolFactory).liquidatorRewardFraction());
